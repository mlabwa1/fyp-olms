...
<button type="submit" id="print" onclick="printPage()">Print</button>	
			<p style = "margin-left:30px; margin-top:50px; font-size:14pt; font-weight:bold;">General Engineering</p>
        <div align="right">
		...
		<br/>
            <?php
            $departmental = "General Engineering";
            //$result= mysqli_query($dbcon,"select * from book") or die (mysqli_error($dbcon));
            $result=mysqli_query($dbcon,"select * from book LEFT JOIN category ON book.category_id = category.category_id where classname='$departmental'")or die(mysqli_error($dbcon));
            while ($row= mysqli_fetch_array ($result) ){
            $id=$row['book_id'];
            $category_id=$row['category_id'];

            $cat_query = mysqli_query($dbcon,"select * from category where classname = '$departmental'")or die(mysqli_error($dbcon));
